...
class DmsfMailer < Mailer
  ...
  def files_updated(user, project, files)
    if user && project && files.count > 0
      ...
      mail :to => user.mail, :from => Setting.mail_from,
  ...
  def send_documents(project, user, email_params)
    ...
    Send by #{User.current.login}@fpt.com.vn"
    
    @body = email_params[:body] + author_mail.html_safe
    ...
    mail :to => email_params[:to], :cc => email_params[:cc],
      :subject => email_params[:subject], 'From' => Setting.mail_from
    ActionMailer::Base.delivery_method = original_delivery_method
  end
